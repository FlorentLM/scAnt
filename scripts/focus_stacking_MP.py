...
def process_stack(threadName, q):
    while not exitFlag_stacking:
        ...
        if not workQueue_stacking.empty():
            ...
            os.system(f"{HUGIN_PATH} -m -x -c 100 -a {temp_output_folder / current_stack}OUT {INPUTS} -v")
...
if __name__ == '__main__':
    ...
    input_images_folder = Path("/home/florent/Desktop/atta_vollenweideri_000139_2/test_project/RAW")
    ...
    if len(usable_images) > 1:
        ...
        [print(path) for path in usable_images]
    else:
    ...
    threadList_stacking = createThreadList(max(nb_stacks, int(num_cores)))
    ...
    queueLock.acquire()
    for stack_name in stacks_names:
        images_current_stack = list(usable_images)
